"use strict";
// 1. Tipos básicos
let tituloSugerencia = "Mejorar la conectividad WiFi";
let descripcionSugerencia = "El internet en la biblioteca es muy lento.";
let esAnonimo = false;
let votos = 15;
let estadoSugerencia = "pendiente";
// 3. Clases
class UsuarioClass {
...
class ComentarioClass {
    constructor(id, autor, mensaje, fecha, anonimo) {
        this.id = id;
        this.autor = autor;
...
class SugerenciaClass {
    constructor(id, titulo, descripcion, autor, comentarios, votos, estado, categoria) {
        this.id = id;
        this.titulo = titulo;
...
// 5. Funciones tipadas
function mostrarSugerencias(sugs) {
    console.log("\n=== SUGERENCIAS ===");
    sugs.forEach(s => {
        console.log(`ID: ${s.id}, Título: ${s.titulo}, Estado: ${s.estado}, Votos: ${s.votos}`);
    });
}
function contarSugerenciasPorEstado(sugs, estado) {
    return sugs.filter(s => s.estado === estado).length;
}
function insertarYEliminarSugerencia(sugs, sugerencia) {
...
const sugerenciaEjemplo = new SugerenciaClass(11, "Implementar reciclaje", "Colocar más tachos de reciclaje en el campus.", usuarios[0], [], 0, "pendiente", "Medio Ambiente");
const baseDatos = {
    usuarios,
    sugerencias,
    comentarios
...
function resumenSugerencia(s) {
    return `Sugerencia: ${s.titulo}\nAutor: ${s.autor.nombre}\nVotos: ${s.votos}\nEstado: ${s.estado}\nComentarios: ${s.comentarios.length}`;
}
function contarComentariosUsuario(usuarioId, comentarios) {
